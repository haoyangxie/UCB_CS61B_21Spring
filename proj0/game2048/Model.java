    ...
    public static boolean maxTileExists(Board b) {
        ...
        for(int row = 0; row < size; row ++) {
            for(int column = 0; column < size; column ++) {
                ...
                else if (b.tile(column, row).value() == MAX_PIECE) {
                    return true;
                }
            }
        }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for(int r = 1; r < size - 1; r ++) {
            for(int c = 1; c < size - 1; c ++) {
                boolean sameValue1 = b.tile(c, r).value() ==  b.tile(c - 1, r).value() || b.tile(c, r).value() == b.tile(c + 1, r).value();
                boolean sameValue2 = b.tile(c, r).value() == b.tile(c, r - 1).value() || b.tile(c, r).value() ==  b.tile(c, r + 1).value();
        ...
        for(int c = 1; c < size - 1; c ++) {
            if(b.tile(c, 0).value() == b.tile(c + 1, 0).value() ||
               b.tile(c, size - 1).value() == b.tile(c + 1, size - 1).value()) {
                return true;
            }
        }
        for(int r = 1; r < size - 1; r ++) {
            if(b.tile(0, r).value() == b.tile(0, r + 1).value() ||
                    b.tile(size - 1, r).value() == b.tile(size - 1, r + 1).value()) {
                return true;
            }
        }
        return false;
